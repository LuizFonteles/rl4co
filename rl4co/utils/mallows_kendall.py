...
def get_expected_distance_P(n,phi):
    #expected distance between the median and 1 random ranking
    P = pu.get_P(n,model='mm_ken',params=phi)
    return np.tril(P,k=-1).sum() 

def expected_dist(P1, P2):
    a = np.triu(P1,1)
    b = np.triu(P2,1)
    return  np.sum(a + b - 2*a*b)
    #return  np.sum(np.triu(P1,1) + np.triu(P2,1) - 2*np.triu(P1,1)*np.triu(P2,1))
    #same as n*(n-1)/2 -np.nansum(P1*P2), which comes from the depth 

def get_expected_dispersion(n,phi):
    #expected distance between 2 random rankings
    P = pu.get_P(n,model='mm_ken',params=phi)
    return np.triu((1-P)*P,k=1).sum() 
...
def prob_numerator(sigma, sigma0, theta=None, phi=None):
    """Probability mass function of a MM with central ranking sigma0 and
    dispersion parameter theta/phi.
    Parameters
    ----------
...
def expected_distance(P,Q):
    return (np.triu(P,1)+np.triu(Q,1)-2*np.triu(P,1)*np.triu(Q,1)).sum()
